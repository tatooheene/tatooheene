...
This example is a stream of costs, therefore, we need the function `apply_discounting`. Using the formulate this gives us:

```{r example-guideline}
apply_discounting(values = v_c_SoC, discount_rate = "costs", times = c(0,1,2))
```

**Please NOTE**: The discount rate for costs in the Netherlands is 3% and is the default discount rate in the function.

@Eline**, misschien hier dan toch in de functie de mogelijkheid geven om dit in een keer te doen, omdat de meeste mensen hier vaak meteen naar op zoek zijn**

You can sum these values using the basic `sum()`.
...
The function now returns the net present values of 150 euro in 4 years time. @Eline **is after hier niet duidelijker dan in ?**
