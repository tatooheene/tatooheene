...
# Understanding assumption about time in discounting.
As you might have noticed, the first cycle is not discounted in the examples above. This is in line with the Dutch guidelines that recommend not to discount the first cycle. With this comes an assumption about when we place rewards on the time axis. When we assume that outcomes occur at the *start of each period*, the first year is not discounted and the time points start at zero, e.g., 0, 1, 2 ,3

Setting up this vector would be: 
```{r example-set-up-times-vector}
v_times             <- seq(from = 0, to = length(v_c_SoC) - 1, by = 1 )
```

However, when outcomes are assumed to occur at the *end of each period*, discounting starts immediately. In that case, vector for times has to start at 1. This can be done with the code below.
...
Or one can assume that outcomes occur halfway through (a compromise between the two). 
...
When reporting results, always state your timing convention clearly (e.g., “End-of-cycle, annual cycles”).
Differences are typically small (fractions of a percent), but transparency ensures that other analysts can reproduce your assumptions and interpret your PVs correctly.
For most models, using a mid-cycle correction provides a balanced approximation between start- and End-of-cycle assumptions.
