...
#@STIJN: @Frederick: misschien ook hier alleen schrijven vanuit het perspectief van ZIN? Geen discounting eerste jaar en dat als uitleg/voorbeeld geven? OF zoals het nu is? Ik vind het langdradig en nog niet heel logsich hoe het nu is. Wellicht hebben jullie een frissere inkijk.

# Understanding assumption about time in discounting.
As you might have noticed, the first cycle is not discounted in the examples above. This is in line with the Dutch guidelines that recommend not to discount the first cycle. With this comes an assumption about when we place rewards on the time axis. When we assume that outcomes occur at the *start of each cycle*, the first year is not discounted and the time points start at zero, e.g., 0, 1, 2 ,3
...
However, when outcomes are assumed to occur at the *end of each cycle*, discounting starts immediately. In that case, vector for times has to start at 1. This can be done with the code below.
...
Or one can assume that outcomes occur halfway through (a compromise between the two), and often applied. 
